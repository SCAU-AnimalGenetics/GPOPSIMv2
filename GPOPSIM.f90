...
SUBROUTINE WELCOMEINF
	...
	call show(io_mode, "          |                  songhl@cau.edu.cn   |")
	call show(io_mode, "          ========================================")
END SUBROUTINE
...
!		end do
 ...
 case(3) !G by E interation

	allocate(gmat(3,3),emat(3,3))
     gmat=0.0;emat=0.0
	 gmat(1,1)=1.0;gmat(2,2)=1.0;gmat(3,3)=alpha1
	 gmat(1,2)=0.5;gmat(1,3)=0.05;gmat(2,3)=0.0
	 gmat(2,1)=gmat(1,2);gmat(3,1)=gmat(1,3);gmat(3,2)=gmat(2,3)
	 emat(1,1)=(1.0/h2(1)-1.0);emat(2,2)=1.0;emat(3,3)=0.25
	 emat(1,2)=0.3;emat(1,3)=0.0;emat(2,3)=0.0
	 emat(2,1)=emat(1,2);emat(3,1)=emat(1,3);emat(3,2)=emat(2,3)
   ...
   do i=1,tqtl
    ...
    do i=1,3
	 ...
	 do j=1,tqtl
		 ...
		 Va_T=Va_T+2.0E0*freq*(1.0E0-freq)*vq(j,i)*vq(j,i)	    
      end do
	    vq(:,i)=sqrt(gmat(i,i)/Va_T)*vq(:,i)
    end do
    ...
    open(unit=21,file="Genetic_paramenters.out",status="replace")
       write(21,"('random variables order: alpha0, Beta, alpha1')")
	   write(21,"('Realized Additive Covariance matrix:')")
	   do i=1,3
	      write(21,"(<3>(f,'	'))") (tvg(i,j),j=1,3)
	   end do
      
       write(21,"('Residual Covariance matrix:')")
	   do i=1,3
	      write(21,"(<3>(f,'	'))") (emat(i,j),j=1,3)
	   end do
...
! control rq 
  if(ntrait>=2 .and.(rq/=0.0 .or. uncommon_QTL==0)) then
    ...
    if(ntrait==2 .and.uncommon_QTL==1)then 
       ...
       if(((rq-0.03)<=corr_rq(1)).and.(corr_rq(1)<=(rq+0.03))) then
   ...
   do i=1,ntrait
	  do j=(i+1),ntrait
        ...
        if(((ra(k)-0.03)<=corr_rq(k)).and.(corr_rq(k)<=(ra(k)+0.03))) then
            continue
        else 
            round=round+1
	        if(round>=10000) then
...
!		write(*,"(3F8.5)") tbv,u*stdve,bpv(i,2)
   ...
   if(ntrait>=2 .and. GbyE/='T') then                              !lixiujin added
	allocate(e(ntrait))
	e=0.0E0
	bpv=0.0E0
  ...
  if(GbyE=='T') then                    !lixiujin added 2020-06-28
	allocate(e(3))
	e=0.0E0; bpv=0.0E0
	 ...
	 ! do k=1,3
	   do j=1,tqtl		! tbv  phenotype
		   genotype=qtype(qtlid(j),2*i-1)+qtype(qtlid(j),2*i)
		   genotype =genotype - 3
		   bpv(i,1,:)=bpv(i,1,:)+ vq(j,:)*genotype  !!ture breeding value
		end do
      ! end do
	 end do
	  
	 do k=1,3       ! scale tbv
       !bpv(:,1,k)=(bpv(:,1,k)-sum(bpv(1:psize,1,k))/real(psize))/sqrt(gmat(k,k))
	   bpv(:,1,k)=bpv(:,1,k)-sum(bpv(1:psize,1,k))/real(psize)
	 end do     


	do i=1,psize 
	  e=0.0E0
	  call multinorm(emat,3,e)
	  bpv(i,2,2)=e(2)+bpv(i,1,2)

	 ! bpv(i,2,1)=bpv(i,1,1)+bpv(i,1,3)*bpv(i,2,2)+e(1)
     ! if(mode_GbyE=='h2') bpv(i,2,1)=e(1)+bpv(i,1,1)+bpv(i,1,3)*bpv(i,2,2)+e(3)*bpv(i,2,2)

      if(trim(mode_GbyE)=='h0') then 
	       bpv(i,2,1)=bpv(i,1,1)+e(1)
      elseif(trim(mode_GbyE)=='h1') then 
           bpv(i,2,1)=bpv(i,1,1)+bpv(i,1,3)*e(2)+e(1)+e(3)*e(2)
	  elseif(trim(mode_GbyE)=='h2') then 
	       bpv(i,2,1)=bpv(i,1,1)+bpv(i,1,3)*bpv(i,2,2)+e(1)+e(3)*bpv(i,2,2)
	  else
	       bpv(i,2,1)=bpv(i,1,1)+bpv(i,1,3)*bpv(i,2,2)+e(1)
	  end if     
...
Subroutine threshold            ! lixiujin added
   implicit none
    ...
    integer          :: i1,i2
     ...
     do i1=1,ntrait
      if(any(i1==T_no)) then 
	      bpv(:,2,i1)=bpv(:,2,i1)-sum(bpv(1:psize,2,i1))/real(psize) !!! scale mean !!
	   do i=1,psize
	loop: do i2=1,nTr(i1) 
   	       if(T(i1,i2-1)<= bpv(i,2,i1).and.bpv(i,2,i1) <T(i1,i2)) then
		     bpv(i,2,i1)=i2-1
			  exit loop
		   end if
	    end do loop
...
if (.false.) then
	do i=1,tq
		...
		if (j1 < 1.0E0) then
			...
			h=1-j1**2-j2**2
			totalh=totalh+h
			if (h/=0) write(43,"(I4,3( F8.4))") i,h,j1,j2
		end if
	end do
	totalh=totalh/tq
	write(43,"(A5, F7.4)") "MeanH",totalh
	write(*,"(A11, F7.4)") "MeanH qtl=",totalh
endif
